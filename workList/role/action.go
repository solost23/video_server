...
func (w *workList.WorkList) AddRoleAuth(casbinModel *model.CasbinModel) error {
	// 先查询本条数据是否存在
	// 若不存在，则插入
...
func (w *workList.WorkList) DeleteRoleAuth(casbinModel *model.CasbinModel) error {
	// 先查询本条数据是否存在
	// 若存在，则删除
	if err := casbinModel.FindByRolePathMethod(casbinModel.RoleName, casbinModel.Path, casbinModel.Method); err != nil {
		return err
...
func (w *workList.WorkList) GetAllRoleAuth(casbinModel *model.CasbinModel) ([]*model.CasbinModel, error) {
	// 直接获取所有
...
func (w *workList.WorkList) GetRoleAuth(casbinModel *model.CasbinModel) (res []*model.CasbinModel, err error) {
	// 直接查找，若找不到，返回错误
